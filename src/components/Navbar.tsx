...
export const Navbar = () => {
    ...
    const navClass = !isSolid
        ...
        : `fixed top-0 left-0 w-full z-[100] px-6 py-6 transition-all duration-500 border-b border-slate-200 shadow-sm ${isMobileMenuOpen ? "bg-white text-slate-900" : "bg-white/95 backdrop-blur-md text-slate-900"
        }`;

    const textClass = !isSolid ? "text-white/80 hover:text-white" : "text-slate-600 hover:text-primary";

    return (
        <>
            <nav className={navClass}>
                <div className="max-w-7xl mx-auto flex items-center justify-between relative z-[102]">
                    <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className={`text-xl md:text-2xl font-bold tracking-tight font-dm-sans transition-colors duration-500 ${!isSolid ? 'text-white' : 'text-primary'}`}>
                        Bagi Kopi
                    </Link>
                ...
                {isMobileMenuOpen && (
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-8">
                            <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-bold tracking-tight font-dm-sans text-primary">
                                Bagi Kopi
                            </Link>
                            <button
                        ...
                        <div className="mt-auto pb-12">
                            <p className="text-sm font-open-sans text-slate-500 mb-4">Connect with us</p>
                            <div className="flex gap-4">
                                <a href="https://instagram.com/bagikopi" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 hover:bg-primary hover:text-white transition-all">
